...
def calcular_fitness(cromosoma):
    ...
    # Verificar restricción: alumnos con notas < 11 no pueden estar todos en el mismo examen
    bajo_rendimiento = [i for i, nota in enumerate(notas) if nota < 11]
    penalizacion = 0
    for examen in ['A', 'B', 'C']:
        indices = asignaciones[examen]
        if all(alumno in indices for alumno in bajo_rendimiento):
            penalizacion -= 1000  # Penalización alta por violar la restricción
...
with open('SESION_08/results-json/resultados_permutacional.json', 'w') as f:
...
print(f"Fitness inicial: {historial_fitness[0]:.4f}")
print(f"Fitness final: {historial_fitness[-1]:.4f}")
print(f"Mejora total: {((historial_fitness[-1] - historial_fitness[0]) / abs(historial_fitness[0]) * 100):.1f}%")
